...
;;     ("lucida 13"
;;      "-b&h-luciatypewriter-medium-r-normal-sans-0-0-0-0-m-0-*-1")
...
;;   recently highlighted region (which is not necessarily the end of te most
...
(defconst hilit-default-face-table
  '(
    ...
    (comment   firebrick-bold   firebrick-bold  italic)
    (include   DarkGreen        nil  bold-italic)
    (define    yellow           nil  bold)
    (defun     blue-bold        nil  bold-italic)
    (decl      black            nil  bold)
    (type      black            nil  nil)
    (keyword   black-bold       nil  bold-italic)
    (label     red              nil  underline)
    (string    grey20           nil  underline)
    ...
    (msg-subject    firebrick-bold  yellow             bold)
    (msg-from      black-bold      green              bold)
    (msg-header            black           cyan               italic)
    (msg-separator  black/tan-bold  black/lightblue    nil)
    (msg-quote     DarkGreen       pink               italic)
  ...
  (let* ((fn (symbol-name face))
    ...
    (cond
     ...
     ((or force
      ...
      ;; JACOB: THIS GIVES "Invalid font or font-face"
